...
[GOPATH](https://github.com/golang/go/wiki/GOPATH), написав в графе значение путь до папки, куда скачан проект {например: C:\Repos\studit\backend} (!!! Обязательно после 3-го и 4-го пунктов перезагрузить консоль, иначе эти изменения не подхватятся)
